<script lang="ts">
  ...
  const nextWordUppercase = $derived(WORD_LIST.find(w => w.toLowerCase() === nextWord)?.toUpperCase() || '');
      ...
      <div class="mt-2 flex flex-col items-center">
        {#if !hasBeenPronounced}
          <button 
            ...
            class="flex items-center gap-2 mb-3 px-4 py-2 bg-blue-100 text-blue-700 rounded-full hover:bg-blue-200 transition-colors"
            disabled={isPlaying}
          >
          ...
          <KeyCap key=" " size="lg" />
          <p class="text-sm text-gray-600 mt-1">Press Space to hear the word</p>
        {:else}
          <KeyCap key=" " size="lg" />
          <p class="text-sm text-gray-600 mt-1">Press Space for next word ({nextWordUppercase})</p>
        {/if}
      </div>
      
      <div class="mt-4 flex gap-4">
        <a href="/" class="text-blue-600 hover:underline text-sm">Back to all words</a>
        <a href="/words/{word.toLowerCase()}" class="text-blue-600 hover:underline text-sm">Practice again</a>
      </div>
    </div>
  </div>
</main>
